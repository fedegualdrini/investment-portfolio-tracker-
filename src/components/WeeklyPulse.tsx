...
const NarrativeSectionView: React.FC<{ section: NarrativeSection }> = ({ section }) => (
  <div className="mb-6">
    ...
    <div className="bg-gray-50/70 dark:bg-gray-800/30 rounded-lg p-4">
      <p className="text-gray-700 dark:text-gray-300 whitespace-pre-line leading-relaxed">{section.body}</p>
    </div>
  </div>
);
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Weekly Pulse</h2>
          <button
        ...
        {narrative ? (
          <div>
            <p className="text-gray-700 dark:text-gray-300 mb-3">{narrative.summary}</p>
            {lastGenerated && (
              <p className="text-sm text-gray-500 dark:text-gray-500">Generated {lastGenerated.toLocaleDateString()}</p>
            )}
          </div>
        ) : (
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-3">
              {onBack && (
                <button onClick={onBack} className="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white transition-colors">
                  ← Back
                </button>
              )}
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{narrative?.title || 'Portfolio Pulse'}</h1>
              <EducationTooltip topicId="diversification" />
            </div>
            {narrative?.date && <p className="text-gray-600 dark:text-gray-400 mt-1">{narrative.date}</p>}
            {lastGenerated && <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">Generated: {lastGenerated.toLocaleString()}</p>}
          </div>
          <button
            onClick={generatePulse}
            disabled={isLoading || investments.length === 0}
            className="flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-lg text-gray-900 dark:text-white font-medium transition-colors"
          >
        ...
        {isLoading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">Analyzing your portfolio...</p>
          </div>
        )}
        ...
        {!isLoading && !narrative && investments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 text-lg">Add some investments to your portfolio to generate your first Pulse.</p>
          </div>
        )}

        {!isLoading && !narrative && investments.length > 0 && (
          <div className="text-center py-12 bg-gray-800/20 border border-gray-700 rounded-lg">
            <p className="text-gray-700 dark:text-gray-300 text-lg mb-3">Ready when you are.</p>
            <p className="text-gray-600 dark:text-gray-400 mb-6">Generate a Pulse to see performance, allocation, news, and educational context.</p>
            <button
              type="button"
              onClick={generatePulse}
