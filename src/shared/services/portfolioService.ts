...
import {
    ...
    calculateTotalAnnualCouponIncome,
    getBonds,
    getStocks,
    getCrypto,
    getCash
                ...
                return {
                    ...
                    results: {
                        ...
                        bestPerformer: investments.reduce((best, inv) => {
                            const currentReturn = (((inv.currentPrice || inv.purchasePrice) - inv.purchasePrice) / inv.purchasePrice) * 100;
                            const bestReturn = (((best.currentPrice || best.purchasePrice) - best.purchasePrice) / best.purchasePrice) * 100;
                            return currentReturn > bestReturn ? inv : best;
                        }),
                        worstPerformer: investments.reduce((worst, inv) => {
                            const currentReturn = (((inv.currentPrice || inv.purchasePrice) - inv.purchasePrice) / inv.purchasePrice) * 100;
                            const worstReturn = (((worst.currentPrice || worst.purchasePrice) - worst.purchasePrice) / worst.purchasePrice) * 100;
